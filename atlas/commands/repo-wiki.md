...
P0环境检测 → 用户配置 → P1变更检测 → P2规划 → P3信息收集 → P4文档生成 → P5索引生成 → P6上下文优化 → P7验证修复

### 用户配置流程

**第一个 AskUserQuestion: 执行模式和文档范围**
...
问题 2: 文档范围
- 完整文档（推荐）: 包含所有模块、API、符号文档
- 核心文档: 仅生成架构和开发指南
- 最小文档: 仅生成首页和架构总览
- 自定义: 指定需要的文档类型
```

**第二个 AskUserQuestion: 文档配置（仅交互模式）**

如果用户选择了**交互模式**，询问详细配置：

```
问题 1: 文档深度
- 深度 2（推荐）: 项目 → 模块 → 类/函数级别
- 深度 1: 项目 → 模块级别
- 深度 3: 包含所有符号的详细信息
- 深度 0: 仅项目级概览
...
问题 3: 符号分析
- 包含符号（推荐）: 提取类、函数、接口等符号信息
- 跳过符号: 仅生成架构和 API 文档

问题 4: 特定功能点
- 自动检测（推荐）: 根据代码自动识别功能模块
- 指定功能: 输入逗号分隔的功能列表（如 auth,payment）
- 跳过功能: 不生成功能文档

问题 5: 并发控制
- 并行模式（推荐）: 最大化并发，快速生成
- 限制并发: 使用 --concurrency 参数指定最大并发数
- 串行模式: 逐个生成，适合调试
```

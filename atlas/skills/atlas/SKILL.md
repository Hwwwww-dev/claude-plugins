...
description: 任务协调和执行框架。用于任务委托、后台处理、并发执行。当用户说"后台执行"、"异步执行"或需要批量操作、项目级变更时使用。让专业 agent 处理任务，支持串行、并行、混合执行策略。触发关键词包括后台执行、异步执行、并发执行、批量、所有、整个项目、重构模块、多文件操作等。
version: 1.0.2
color: pink
---
...
**核心理念**：当用户说"后台执行"、"异步执行"时，意味着他们想把任务委托给 Atlas 框架，而不是在主线程直接处理。
...
**典型触发词**：
- **任务委托**: "后台执行"、"异步执行"、"交给 agent"、"让 Atlas 处理"
- **批量操作**: "所有"、"批量"、"全部"、"每个"
